...
2026-08-01 先冻结以关联组为主的 `wpf-page-set-v1`，随后按“结构完整、复杂度适中、前后端边界清晰”的规则扩充为正式版本 `wpf-page-set-v2`。v2 覆盖全部 20 个正式项目，共 73 页、688 个控件和 35 条集合内页面边；52 页参与联动，21 页为独立对照，其中 20 页不超过 10 个控件。逐项目选择、硬排除规则、代表性反例、源码证据和实验边界见[WPF 迁移实验冻结页面集合](experiment-page-set.md)，机器可读输入见 [`experiment-page-set-v2.json`](experiment-page-set-v2.json)，[`experiment-page-set-v1.json`](experiment-page-set-v1.json) 仅用于版本追溯。
