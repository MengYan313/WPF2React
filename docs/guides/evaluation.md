...
正式 20 个项目是广义语料库，不是默认共享一个权重的单一测试集。项目可按领域、项目形态、页面规模和迁移挑战分层报告；当前机器可读统计由 `scripts/analyze_dataset_stats.py` 写入 `results/dataset/dataset-statistics.json`。

每次正式实验使用同一份 page ID 清单和抽样理由。主业务、框架、Gallery 与平台专项分别报告；压力集按页面规模、根类型、自定义控件比例和未决依赖分层抽样，不能只选择容易页面。聚合时先计算仓库内结果，再对仓库和类别做宏平均；页面数加权的微平均只作为补充。
...
- `visual_evaluation_records.jsonl`：每行一个截图对，包含图片路径、分项证据、问题严重度与建议。
...
LLM 视觉评分具有主观性。正式实验宜随机抽取一部分样本由至少两名人工评审独立打分，报告人与人、人与模型的一致性，并使用同一模型和截图清单。
