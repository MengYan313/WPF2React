...
Parser 的仓库相对 `source_id/page_id` 会贯穿完整迁移流程：TypeScript 产物镜像源码相对目录，类型 import 按该目录生成，页面的根/子合同按 `page_id` 对应的依赖关系确定。页面最终组装必须重新确认 Props 合同并校验本地 import，只有引用模块真实存在且最终合同通过时才记录成功。
