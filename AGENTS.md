...
- Recreate the verified macOS arm64 environment with `python3.11 -m venv .venv` and `.venv/bin/python -m pip install -r requirements-local-macos-arm64.lock.txt`.
...
The current runtime intentionally uses only `OPENAI_MODEL_LOW` through `LLMConfig.marker_mode()`. Medium and high are reserved for later explicit routing decisions. `text-embedding-3-small` in `MUISelectAgent` is an embedding model, not a generative LLM tier.
