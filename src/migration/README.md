...
新 Parser 产物同时提供兼容的基础控件树 `controls` 和用于实际迁移的 `migration_controls`。后者增加可视自建控件，但不会改变冻结实验沿用的基础控件统计口径。完整知识层、目标版本约束、召回指标和 smoke 结果见[组件知识库设计与验证](../../docs/guides/component-knowledge-base.md)。
