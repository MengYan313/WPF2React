...
def main() -> bool:
    ...
    default_config = LLMConfig.json_mode_config()
    ...
    success = success and default_config.json_mode

    print(f"model_tiers_ok={resolved == EXPECTED_MODELS}")
    print(f"runtime_uses_low={default_config.model == EXPECTED_MODELS['low']}")
    print(f"json_mode_ok={default_config.temperature == 0 and default_config.json_mode}")
    return success
